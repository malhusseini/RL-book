...
class CalibrationParams:
    ...
    """
    # Unconditional (used when no regime in MDP)
    mu_1: float
    sigma_1: float
    mu_2: float
    sigma_2: float
    ...
    # Regime-conditional (None if regime not used)
    ...
    # Row 0 = bad, Row 1 = good  (matching regime_int encoding in mdp.py)
...
def _fit_bivariate(
    ...
    """
    Fit (mu1, sigma1, mu2, sigma2, rho) from two aligned return series.
    Uses only the rows where both series have valid (non-NaN) values.
    """
    ...
    # Guard against degenerate correlation (e.g. constant series)
...
def assign_regime(
    ...
    Args:
        market_returns:    Daily market return series (e.g. vwretd), date-indexed.
        window:            Short window for realized vol estimate (default 21 days).
        long_window_mult:  Long window = window * long_window_mult for the median.

    Returns:
        Boolean Series indexed by date: True = good regime, False = bad.
    """
    ...
    regime = roll_vol < roll_median   # True = low vol = good
    return regime.rename("regime_good")


def _build_transition_matrix(regime_series: pd.Series) -> np.ndarray:
    """
    Estimate the empirical 2×2 Markov transition matrix from consecutive
    daily regime assignments.

    Encoding:  0 = bad, 1 = good
    Entry [i, j] = estimated P(next_regime = j | curr_regime = i).
    """
    regime_int = regime_series.astype(int).values   # 1 = good, 0 = bad
    counts = np.zeros((2, 2), dtype=float)
    for curr, nxt in zip(regime_int[:-1], regime_int[1:]):
        counts[curr, nxt] += 1.0

    row_sums = counts.sum(axis=1, keepdims=True)
    row_sums[row_sums == 0] = 1.0     # avoid divide-by-zero for unseen regimes
...
def calibrate(
    ...
    if include_regime and market_col in strategy_series.columns:
        ...
        # Align regime with strategy returns (inner join on date)
        ...
        # Regime-conditional bivariate fits
